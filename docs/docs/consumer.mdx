...
1. `connectWithSecret()`, for authenticating with a client ID and secret, typically used for machine-to-machine services.1
1. `connectWithIdentity()`, for authenticating with an Azure managed identity.
1. `connectWithAccessToken()` for authenticating an access token with SSL/TLS.
1. `connectWithAccessTokenInsecure()` for authenticating using an access token without SSL/TLS for testing.
...
// With an personal access token and SSL
try (GrpcChannel channel = Connect.connectWithAccessToken(
    "example.zepben.com", // rpc hostname
    443,                  // rpc port
    "your_access_token",  // personal access token generated from EAS
...
// With an personal access token without SSL
