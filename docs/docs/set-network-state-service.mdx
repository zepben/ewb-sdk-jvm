---
id: set-network-state-service
title: Set Network State Service
---
...
The `onSetCurrentStates` callback is triggered for each request passing in a batch of current state events, and should return a `SetCurrentStatesStatus` to
reflect the success or failure of the update process.
...
Function<List<CurrentStateEvent>, SetCurrentStatesStatus> updateCurrentStatesLambda  = (events) -> {
    // process updating of events here and return a `SetCurrentStatesStatus`
};
