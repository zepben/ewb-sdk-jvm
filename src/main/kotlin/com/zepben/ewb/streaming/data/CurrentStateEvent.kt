...
import com.zepben.ewb.services.network.translator.mapPhaseCode
...
sealed class CurrentStateEvent(
    ...
    companion object {
        ...
        internal val mapSwitchAction = EnumMapper(SwitchAction.entries, PBSwitchAction.entries)
    }
...
class SwitchStateEvent @JvmOverloads constructor(
    ...
    override fun toPb(): PBCurrentStateEvent = toPbBuilder().also { event ->
        event.switch = PBSwitchStateEvent.newBuilder().also {
            it.mrid = mRID
            it.action = mapSwitchAction.toPb(action)
            it.phases = mapPhaseCode.toPb(phases)
        }.build()
    }.build()
