...
internal class GrpcChannelBuilderTest {
        ...
        grpcChannel = GrpcChannelBuilder().forAddress("hostname", 1234)
            .build(
                GrpcBuildArgs(
                    ...
                    debugConnectionTest = false,
                    connectionTestTimeoutMs = 5000,
                    maxInboundMessageSize = DEFAULT_BUILD_ARGS.maxInboundMessageSize
                )
            )
    ...
    internal fun `testConnection collects other exceptions for debug output`() {
        ...
        }.toThrow<GrpcConnectionException>().withMessage(
            "Couldn't establish gRPC connection to any service on localhost:50051.\n" +
            ...
            GrpcChannelBuilder().forAddress("hostname", 1234).makeSecure(caFile, certChainFile, pkFile)
                .build(
                    GrpcBuildArgs(
                        ...
                        debugConnectionTest = false,
                        connectionTestTimeoutMs = 5000,
                        maxInboundMessageSize = 53
                    )
                ).channel, equalTo(secureChannel)
        )
        assertThat(
    ...
    internal fun withTokenFetcher() {
        ...
        mockkStatic(NettyChannelBuilder::class)
        every {
            NettyChannelBuilder.forAddress("hostname", 1234, any()).maxInboundMessageSize(any()).intercept(any<CallCredentialApplier>()).build()
        } returns authenticatedChannel
