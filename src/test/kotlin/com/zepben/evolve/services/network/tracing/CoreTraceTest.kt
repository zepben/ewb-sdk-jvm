...
internal class CoreTraceTest {
    ...
    private fun currentNonDirectionalTrace(start: ConductingEquipment, phases: PhaseCode): List<TrackedPhases> {
        return runTrace(Tracing.connectedEquipmentTrace(NetworkStateOperators.CURRENT).addNetworkCondition { stopAtOpen() }, start, phases)
    }

    private fun normalDownstreamTrace(start: ConductingEquipment, phases: PhaseCode): List<TrackedPhases> {
        return runTrace(Tracing.connectedEquipmentTrace(NetworkStateOperators.NORMAL).addNetworkCondition { downstream() }, start, phases)
    }

    private fun currentDownstreamTrace(start: ConductingEquipment, phases: PhaseCode): List<TrackedPhases> {
        return runTrace(Tracing.connectedEquipmentTrace(NetworkStateOperators.CURRENT).addNetworkCondition { downstream() }, start, phases)
    }

    private fun normalUpstreamTrace(start: ConductingEquipment, phases: PhaseCode): List<TrackedPhases> {
        return runTrace(Tracing.connectedEquipmentTrace(NetworkStateOperators.NORMAL).addNetworkCondition { upstream() }, start, phases)
    }
