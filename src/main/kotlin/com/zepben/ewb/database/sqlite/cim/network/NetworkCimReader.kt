...
internal class NetworkCimReader : CimReader<NetworkService>(), AutoCloseable {
    ...
    @Throws(SQLException::class)
    private fun readContactDetails(
        table: TableContactDetails,
        resultSet: ResultSet,
        setIdentifier: (String) -> String
    ): ContactDetails =
        ContactDetails(setIdentifier(resultSet.getString(table.ID.queryIndex))).apply {
    ...
    private fun readStreetDetail(table: TableStreetAddresses, resultSet: ResultSet): StreetDetail? =
        StreetDetail(
            ...
            resultSet.getNullableString(table.BUILDING_NUMBER.queryIndex)?.internEmpty(),
    ...
    private fun readTownDetail(table: TableTownDetails, resultSet: ResultSet): TownDetail? =
        TownDetail(
            ...
            resultSet.getNullableString(table.COUNTRY.queryIndex)?.internEmpty(),
        ).takeUnless { it.allFieldsNull() }
    ...
    fun read(service: NetworkService, table: TableUsagePointsContactDetails, resultSet: ResultSet, setIdentifier: (String) -> String): Boolean {
        ...
        service.getOrThrow<UsagePoint>(usagePointMRID, "UsagePoint to ContactDetails association").apply {
            addContact(readContactDetails(table, resultSet, setIdentifier))
        }
