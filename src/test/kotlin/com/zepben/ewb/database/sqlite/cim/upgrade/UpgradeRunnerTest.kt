...
import com.zepben.ewb.database.sqlite.extensions.configureBatch
...
//            assertThat(actualChangeSets.map { it.number }, equalTo((actualChangeSets.minOf { it.number }..TableVersion().supportedVersion).toList()))
        UpgradeRunner().apply {
    ...
    internal fun `configures connection for batch processing`() {
        mockkStatic("com.zepben.ewb.database.sqlite.extensions.ConnectionExtensionsKt") {
    ...
    internal fun `reports obsolete database version`() {
        mockkStatic("com.zepben.ewb.database.sql.extensions.StatementExtensionsKt") {
