...
object Connect {
    ...
    /**
     * Create a `GrpcChannel` that communicates with the gRPC service using SSL/TLS transport security and an access token.
     *
     * @param host The hostname where the gRPC service is hosted
     * @param rpcPort The port of the gRPC service
     * @param token The access token generated by the Evolve application
     * @param caFilename The filename of a truststore containing additional trusted root certificates. This parameter is optional
     *                   and defaults to null, in which case only the system CAs are used to verify certificates.
     * @return An encrypted connection to the gRPC service
     */
    ...
    fun connectWithAccessToken(
        ...
        return connectWithTokenString(token, host, rpcPort, caFilename)
    }
    ...
    private fun connectWithTokenString(
        token: String,
        host: String,
        rpcPort: Int,
        caFilename: String?
    ): GrpcChannel {
        return GrpcChannelBuilder().forAddress(host, rpcPort).makeSecure(rootCertificates = caFilename).withTokenString(token).build()
    }
