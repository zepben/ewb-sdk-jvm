...
internal class QueryNetworkStateServiceTest {
    ...
    private fun QueryNetworkStateService.assertGetCurrentStates(onAdditionalVerification: (GetCurrentStatesRequest) -> Unit = {}) {
        ...
        fun GetCurrentStatesResponse.validate(expectedBatch: CurrentStateEventBatch) {
            ...
            assertThat(eventList.map { it.switch.action.name }, contains(*expectedSwitchEvents.map { endsWith(it.action.name) }.toTypedArray()))
            assertThat(eventList.map { it.switch.phases.name }, contains(*expectedSwitchEvents.map { endsWith(it.phases.name) }.toTypedArray()))
        }
